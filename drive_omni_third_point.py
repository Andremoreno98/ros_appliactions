...
class DriveOmniThirdPoint(EventState):
    '''
    ...
    def __init__(self):
        super(DriveOmniThirdPoint, self).__init__(outcomes=['failed', 'done'])
        ...
        self.pub_third = ProxyPublisher({self.vel_topic: Twist})
    ...
    def execute(self, userdata):
        ...
        if self.initial_x != 0.0 and self.initial_y != 0.0:
            ...
            goal_x = start_x
            start_y = math.floor(self.list_odo_y[0])
            ...
            if (distance_to_drive - abs(distance_travelled)) > 0.02: #decreasing the difference adjust the accuracy of the last position
                ...
                self.pub_third.publish(self.vel_topic, self.cmd_pub)
            else:
    ...
    def on_enter(self, userdata):
        ...
        self.cmd_pub = Twist()
        # create subscriber
        self.sub_odo_third = rospy.Subscriber("/odometry/filtered", Odometry, self.newOdom)
